...
class TestNotificationServiceReportGeneration(unittest.TestCase):
    ...
    @mock.patch("src.notification.get_config")
    def test_generate_brief_report_uses_management_summary_when_summary_only_enabled(
        self, mock_get_config: mock.MagicMock
    ):
        mock_get_config.return_value = _make_config(
            report_renderer_enabled=True,
            report_summary_only=True,
            report_scene_label="早报",
            web_report_base_url="https://dsa.example.com",
        )
        service = NotificationService()
        result = AnalysisResult(
            code="600519",
            ...
            sentiment_score=82,
            trend_prediction="看多",
            operation_advice="逢回调分批买入",
            analysis_summary="龙头趋势仍在，回调承接稳定。",
            decision_type="buy",
            query_id="rec-123",
            dashboard={
                "core_conclusion": {
                    "one_sentence": "龙头趋势仍在，回调承接稳定。",
                    "time_sensitivity": "先看量价能否继续配合。",
                },
                "battle_plan": {
                    "action_checklist": ["观察量能是否继续放大"],
                    "sniper_points": {"ideal_buy": "1550-1580"},
                },
                "intelligence": {
                    "risk_alerts": ["消费情绪若转弱，短线波动会放大"],
                },
            },
        )

        with mock.patch("src.services.report_renderer.render") as mock_render:
            out = service.generate_brief_report([result], report_date="2026-04-02")

        mock_render.assert_not_called()
        self.assertIn("早报决策简报", out)
        self.assertIn("管理层摘要", out)
        self.assertIn("能不能做", out)
        self.assertIn("为什么", out)
        self.assertIn("风险在哪", out)
        self.assertIn("接下来盯什么", out)
        self.assertIn("https://dsa.example.com/?queryId=rec-123", out)

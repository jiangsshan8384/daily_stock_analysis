...
class NotificationService(
    ...
    def _get_report_scene_label(self) -> str:
        return (getattr(get_config(), 'report_scene_label', '') or '').strip()

    def _truncate_text(self, text: Any, limit: int = 60, fallback: str = "暂无") -> str:
        value = str(text or '').strip()
        if not value:
            return fallback
        return value if len(value) <= limit else value[:limit].rstrip() + '...'

    def _get_management_action(self, result: AnalysisResult, report_language: str) -> str:
        advice = (getattr(result, 'operation_advice', '') or '').strip()
        decision_type = (getattr(result, 'decision_type', '') or '').strip().lower()
        if report_language == 'en':
            if decision_type == 'buy' or any(k in advice.lower() for k in ('buy', 'accumulate', 'add')):
                return 'Can act, but follow the plan in batches.'
            if decision_type == 'sell' or any(k in advice.lower() for k in ('sell', 'reduce', 'take profit')):
                return 'Do not force it now; manage risk first.'
            return 'Keep watching first; wait for confirmation before acting.'

        if decision_type == 'buy' or any(k in advice for k in ('买', '加仓', '建仓')):
            return '可以做，但按计划分批，不要追高。'
        if decision_type == 'sell' or any(k in advice for k in ('卖', '减仓', '止盈', '止损')):
            return '先别硬做，优先把风险收住。'
        return '可以继续盯，但先等确认信号，不急着出手。'

    def _build_web_detail_url(self, result: AnalysisResult) -> Optional[str]:
        base_url = (getattr(get_config(), 'web_report_base_url', '') or '').strip()
        query_id = (getattr(result, 'query_id', '') or '').strip()
        if not base_url or not query_id:
            return None
        return f"{base_url.rstrip('/')}/?queryId={quote(query_id)}"

    def _extract_management_summary(self, result: AnalysisResult) -> Dict[str, str]:
        ...
        reason = self._truncate_text(
            core.get('one_sentence') or getattr(result, 'analysis_summary', ''),
            limit=72,
            fallback='暂无明确结论',
        )
        risk = self._truncate_text(
            (risks[0] if risks else None) or getattr(result, 'risk_warning', ''),
            limit=56,
            fallback='暂未看到新增硬风险，但仍要控制仓位。' if report_language == 'zh' else 'No new hard risk flagged yet, but keep risk control in place.',
        )

        focus_candidate = None
        if checklist:
            focus_candidate = checklist[0]
        elif core.get('time_sensitivity'):
            focus_candidate = core.get('time_sensitivity')
        elif sniper.get('ideal_buy'):
            focus_candidate = (
                f"先看是否回到理想买点 {sniper.get('ideal_buy')} 再动。"
                if report_language == 'zh'
                else f"Watch whether price revisits the ideal entry {sniper.get('ideal_buy')} before acting."
            )
        elif sniper.get('stop_loss'):
            focus_candidate = (
                f"先守住风控位 {sniper.get('stop_loss')}。"
                if report_language == 'zh'
                else f"Keep the stop level {sniper.get('stop_loss')} in focus first."
            )

        focus = self._truncate_text(
            focus_candidate,
            limit=56,
            fallback='盯住价格是否继续走强、风险消息有没有增多。' if report_language == 'zh' else 'Watch whether price keeps improving and whether risk headlines increase.',
        )

        return {
            'action': self._get_management_action(result, report_language),
            'reason': reason,
            'risk': risk,
            'focus': focus,
            'detail_url': self._build_web_detail_url(result) or '',
        }

    def _generate_management_brief_report(
        self,
        results: List[AnalysisResult],
        report_date: str,
    ) -> str:
        ...
        scene_label = self._get_report_scene_label()
        brief_title = f"{scene_label}{labels['brief_title']}" if scene_label else labels['brief_title']

        if not results:
            return f"# {report_date} {brief_title}\n\n{labels['no_results']}"
        ...
        if report_language == 'en':
            summary_heading = 'Management View'
            action_label = 'Can Act?'
            reason_label = 'Why'
            risk_label = 'Risk'
            focus_label = 'Watch Next'
            detail_label = 'Web Detail'
        else:
            summary_heading = '管理层摘要'
            action_label = '能不能做'
            reason_label = '为什么'
            risk_label = '风险在哪'
            focus_label = '接下来盯什么'
        ...
        for result in sorted_results:
            ...
            summary = self._extract_management_summary(result)
            lines.extend([
                f"### {emoji} {self._get_display_name(result, report_language)}({result.code})",
                f"- **{action_label}**：{summary['action']}",
                f"- **{reason_label}**：{summary['reason']}",
                f"- **{risk_label}**：{summary['risk']}",
                f"- **{focus_label}**：{summary['focus']}",
            ])
            if summary['detail_url']:
                lines.append(f"- **{detail_label}**：{summary['detail_url']}")
            lines.append("")
    ...
    def generate_dashboard_report(
        ...
        labels = get_report_labels(report_language)
        scene_label = self._get_report_scene_label()
        effective_summary_only = self._report_summary_only if summary_only is None else summary_only
        ...
        dashboard_title = f"{scene_label}{labels['dashboard_title']}" if scene_label else labels['dashboard_title']
        report_lines = [
            f"# 🎯 {report_date} {dashboard_title}",
            "",
            f"> {labels['analyzed_prefix']} **{len(results)}** {labels['stock_unit']} | "
            f"🟢{labels['buy_label']}:{buy_count} 🟡{labels['watch_label']}:{hold_count} 🔴{labels['sell_label']}:{sell_count}",
            "",
    ...
    def generate_brief_report(
        ...
        if effective_summary_only:
            return self._generate_management_brief_report(results, report_date)
